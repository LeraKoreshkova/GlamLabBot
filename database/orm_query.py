...
from database.models import Product, Category, Subcategory, Color, Banner, Cart, User
...
async def orm_get_category(*, session: AsyncSession, subcategory_id: int):
    query = select(Subcategory.category_id).where
    result = await session.execute(query)
    return result.scalars().all()
...
async def orm_get_subcategories(*, session: AsyncSession, category_id: int):
    query = select(Subcategory).where(Subcategory.category_id == category_id)
    result = await session.execute(query)
    return result.scalars().all()


async def orm_create_subcategories(*, session: AsyncSession, subcategories: list, category_id: int):
    query = select(Subcategory)
    result = await session.execute(query)
    # if result.first():
    #     return
    session.add_all([Subcategory(name=name, category_id=category_id) for name in subcategories])
    await session.commit()


############################ Цвета ######################################

async def orm_get_colors(*, session: AsyncSession):
    query = select(Color)
    result = await session.execute(query)
    return result.scalars().all()


async def orm_create_colors(*, session: AsyncSession, colors: list):
    query = select(Color)
    result = await session.execute(query)
    if result.first():
        return
    session.add_all([Color(color=color) for color in colors])
    await session.commit()
...
async def orm_get_products(*, session: AsyncSession, subcategory_id: int):
