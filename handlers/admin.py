...
from database.orm_query import orm_get_products, orm_add_product, orm_delete_product, orm_get_product, \
    orm_get_categories, orm_get_subcategories, orm_update_product, orm_get_info_pages, orm_change_banner_image
...
async def starring_at_subcategory(callback: types.CallbackQuery, session: AsyncSession):
    category_id = callback.data.split('_')[-1]
    await callback.answer('')
    subcategories = await orm_get_subcategories(session=session, category_id=int(category_id))
    btns = {subcategory.name: f'subcategory_{subcategory.id}' for subcategory in subcategories}
    await callback.message.edit_text("Выберите подкатегорию", reply_markup=get_callback_btns(btns=btns))


@admin_router.callback_query(F.data.startswith('subcategory_'))
async def starring_at_product(callback: types.Message, session: AsyncSession):
    subcategory_id = callback.data.split('_')[-1]
    await callback.message.delete()
    for product in await orm_get_products(session=session, subcategory_id=int(subcategory_id)):
...
#Доработать функцию удаления товаров
@admin_router.callback_query(F.data.startswith("delete_"))
async def delete_product_callback(callback: types.CallbackQuery, session: AsyncSession):
    product_id = callback.data.split("_")[-1]
    await callback.answer("Уверен?", show_alert=True)
...
async def add_banner(message: types.Message, state: FSMContext, session: AsyncSession):
    ...
    if for_page not in pages_names:
        await message.answer(f"Введите нормальное название страницы, например:\
                         \n{', '.join(pages_names)}")
        return
    await orm_change_banner_image(session=session, name=for_page, image=image_id)
...
async def add_name(message: types.Message, state: FSMContext):
    if message.text == "." and AddProduct.product_for_change:
        ...
        # например:
        if 4 >= len(message.text) >= 200:
            await message.answer(
                "Название товара не должно превышать 200 символов или быть менее 5ти символов. \n Введите заново"
            )
            return
...
async def add_description(message: types.Message, state: FSMContext, session: AsyncSession):
    ...
    await message.answer(f"Выберите категорию\n"
                         f"{data}", reply_markup=get_callback_btns(btns=btns))
...
async def category_choice(callback: types.CallbackQuery, state: FSMContext, session: AsyncSession):
    if int(callback.data) in [category.id for category in await orm_get_categories(session=session)]:
        await callback.answer()
        await state.update_data(category=callback.data)
        data = await state.get_data()
        subcategories = await orm_get_subcategories(session=session, category_id=int(data['category']))
        btns = {subcategory.name: str(subcategory.id) for subcategory in subcategories}
        await callback.message.answer(f"Выберите подкатегорию\n"
                                      f"{data}", reply_markup=get_callback_btns(btns=btns))
...
# Ловим callback выбора подкатегории
...
async def subcategory_choice(callback: types.CallbackQuery, state: FSMContext, session: AsyncSession):
    data = await state.get_data()
    if int(callback.data) in [subcategory.id for subcategory in await orm_get_subcategories(session=session, category_id=int(data['category']))]:
        await callback.answer()
        await state.update_data(subcategory=callback.data)
        data = await state.get_data()
        await callback.message.answer(f"Теперь введите цену товара.\n"
                                      f"{data}")
        await state.set_state(AddProduct.price)
    else:
        await callback.message.answer('Выберите подкатеорию из кнопок.')
        await callback.answer()

#Ловим любые некорректные действия, кроме нажатия на кнопку выбора подкатегории
@admin_router.message(AddProduct.subcategory)
async def subcategory_choice2(message: types.Message, state: FSMContext):
    await message.answer("'Выберите подкатеорию из кнопок.'")
...
async def add_price(message: types.Message, state: FSMContext):
    ...
    data = await state.get_data()
    await message.answer(f"Загрузите изображение товара\n"
                         f"{data}")
...
async def add_image(message: types.Message, state: FSMContext, session: AsyncSession):
    ...
    try:
        ...
        await message.answer(f"Товар добавлен/изменен\n"
                             f"{data}", reply_markup=ADMIN_KB)
        await state.clear()
